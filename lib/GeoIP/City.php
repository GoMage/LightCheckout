...
static function getrecordwithdnsservice($str){
  ...
  $number = GeoIP_Core::$GEOIP_COUNTRY_CODE_TO_NUMBER[$record->country_code];
  $record->country_code3 = GeoIP_Core::$GEOIP_COUNTRY_CODES3[$number];
  $record->country_name = GeoIP_Core::$GEOIP_COUNTRY_NAMES[$number];
  if ($record->region != "") {
...
static function _get_record($gi,$ipnum, GeoIP_Core $geoip){
  ...
  if ($gi->flags & GeoIP_Core::GEOIP_MEMORY_CACHE) {
    $record_buf = substr($gi->memory_buffer,$record_pointer,self::FULL_RECORD_LENGTH);
  } elseif ($gi->flags & GeoIP_Core::GEOIP_SHARED_MEMORY){
  ...
  if (GeoIP_Core::GEOIP_CITY_EDITION_REV1 == $gi->databaseType){
    $metroarea_combo = 0;
...
static function GeoIP_record_by_addr ($gi,$addr,GeoIP_Core $geoip){
  if ($addr == NULL){
     return 0;
  }
  $ipnum = ip2long($addr);
