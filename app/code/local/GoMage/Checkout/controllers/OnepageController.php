...
class GoMage_Checkout_OnepageController extends Mage_Checkout_Controller_Action {
			...
			switch ($action) :
				...
				case ('giftcard'):
					...
					try {
						...
						if ($remove){
							$result->message = $this->__('Gift Card "%s" was removed.', Mage::helper('core')->htmlEscape($giftcard_code));
						} else {
							$result->message = $this->__('Gift Card "%s" was added.', Mage::helper('core')->htmlEscape($giftcard_code));
						}
				...
				case ('cartremove'):
					...
					if ($itemId = $this->getRequest()->getParam('id')) {
						...
						if (! $this->getOnepage()->getQuote()->isVirtual()) {
							...
							$result->methods = $this->_getMethodsHtml();
						}
				...
				case 'prepare_sagepay':
					...
					if ((bool)($this->getSession()->getCustomer()->getId()) == false && 
						(intval(Mage::helper('gomage_checkout')->getCheckoutMode()) === 1 || 
						 $this->getRequest()->getParam('create_account') ||
						 Mage::helper('gomage_checkout')->getConfigData('registration/auto'))) {
	...
	protected function _getMethodsHtml() {
		$layout = Mage::getModel('core/layout');
		$layout->getUpdate()->load('gomage_checkout_onepage_methods');
		$layout->generateXml()->generateBlocks();
		return $layout->getOutput();
	}
